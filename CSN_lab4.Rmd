...
```{r}
library(robustbase)
robust_3 <- nlrob(degree_2nd_moment~a*exp(c*vertices), data=Catalan, start = list(a = a_linreg, c = c_linreg))
end1 <- Sys.time()
coef(robust_3)[1]
coef(robust_3)[2]
end1-start1
...
```{r}
resid = robust_3$residuals
fitted = robust_3$fitted
plot(fitted, resid)
```
...
# reg
nonlinear_model = nls(Mean_k2m~(vertices)^b, data= grouped_Catalan, start = list(b = b_linreg))
